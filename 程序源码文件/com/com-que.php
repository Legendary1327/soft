...
<!--                            <th>处理结果</th>-->
                        ...
                        $sql = "select req_time, car_name, req_cargo_num, req_cargo_dest_comp from cargo_request LEFT JOIN 
cargo_info on cargo_request.req_cargo_id = cargo_info.car_id where req_req_user_id = '$id' and req_result = 0 order by req_cargo_dest_comp;";
                        ...
                        if ($number != 0) {
                            for($i = 0; $i < $number; $i++) {
                                ...
                                $str_car = '<tr>
<td>' . $row['req_time'] . '</td><td>' . $row['car_name'] . '</td><td>' . $row['req_cargo_num'] . '</td>
<td>' . $row['req_cargo_dest_comp'] . '</td>
...
<script>
                    ...
                    <table class="tab_cargo_info" >
                        ...
                        $sql = "select req_time, car_name, req_cargo_num, req_cargo_dest_comp, req_result, req_jud_time from cargo_request LEFT JOIN 
cargo_info on cargo_request.req_cargo_id = cargo_info.car_id where req_req_user_id = '$id' and req_result <> 0;";
                        $result = $conn->query($sql);
                        $number = mysqli_num_rows($result);
...
<tr>
<td>' . $row['req_time'] . '</td><td>' . $row['car_name'] . '</td><td>' . $row['req_cargo_num'] . '</td>
<td>' . $row['req_cargo_dest_comp'] . '</td><td>' . $row['req_result'] . '</td><td>' . $row['req_jud_time'] . '</td>
</tr>
<script>
</script>';
                                echo $str_car;
